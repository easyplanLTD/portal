...
// Kept identical to FixFlow admin's own APPLIANCE_TYPES / BRANDS (src/App.jsx) so an
// engineer's Skills tab here always matches what admin sees/sets on the Edit Engineer
// modal — same appliance list, same order, same per-appliance brand options.
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: C.sidebar, color: C.text }}>
...
const PORTAL_NAV = [
  ...
  { key: "bookings", label: "Bookings", ic: "📋" },
  { key: "payments", label: "Payments", ic: "£" },
  { key: "support", label: "Support", ic: "🛟" },
      ...
      <nav className="flex-1 py-4 space-y-1 px-3">
        {PORTAL_NAV.map((n) => {
          ...
          return (
            ...
            >
              <span className="mr-2">{n.ic}</span>{n.label}
            </button>
          );
        })}
      </nav>
    ...
    >
      {PORTAL_NAV.map((n) => {
        ...
        return (
          ...
          >
            <span className="text-lg leading-none">{n.ic}</span>
...
function PaymentsView({ currentUser, jobs }) {
  ...
  const unsettledEarnings = unpaidJobs.reduce((s, j) => s + currentUser.payRate, 0);
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="rounded-xl p-5" style={{ background: C.card, borderLeft: `4px solid ${C.warn}` }}>
          <p className="text-sm" style={{ color: C.mid }}>Earnings (unsettled)</p>
          <p className="text-2xl font-bold" style={{ color: C.warn }}>{fmtMoney(unsettledEarnings)}</p>
        </div>
        <div className="rounded-xl p-5" style={{ background: C.card, borderLeft: `4px solid ${C.warn}` }}>
...
function SettingsView({ currentUser, onAddTimeOff, onRemoveTimeOff, onProfileFieldsChanged }) {
  ...
  const [applianceTypes, setApplianceTypes] = useState(currentUser.applianceTypes || []);
  const [brandExclusions, setBrandExclusions] = useState(currentUser.brandExclusions || {});
  const [skillsSaving, setSkillsSaving] = useState(false);
  const [skillsSaved, setSkillsSaved] = useState(false);
  const [postcodes, setPostcodes] = useState(currentUser.postcodes || []);
  const [newPostcode, setNewPostcode] = useState("");
  const [postcodesSaving, setPostcodesSaving] = useState(false);
  const [postcodesSaved, setPostcodesSaved] = useState(false);

  function toggleAppliance(type) {
    setApplianceTypes((types) => (types.includes(type) ? types.filter((t) => t !== type) : [...types, type]));
    setSkillsSaved(false);
  }

  function toggleBrandExclusion(type, brand) {
    setBrandExclusions((excl) => {
      const current = excl[type] || [];
      return { ...excl, [type]: current.includes(brand) ? current.filter((b) => b !== brand) : [...current, brand] };
    });
    setSkillsSaved(false);
  }
  ...
  async function saveSkills() {
    ...
    const { error } = await supabase.from("engineers").update({ appliance_types: applianceTypes, brand_exclusions: brandExclusions }).eq("profile_id", currentUser.id);
    setSkillsSaving(false);
    if (!error) { setSkillsSaved(true); onProfileFieldsChanged?.({ applianceTypes, brandExclusions }); }
  }

  function addPostcode() {
    const pc = newPostcode.trim().toUpperCase();
    if (pc && !postcodes.includes(pc)) setPostcodes((p) => [...p, pc]);
    setNewPostcode("");
    setPostcodesSaved(false);
  }
  ...
  async function savePostcodes() {
    setPostcodesSaving(true);
    const { error } = await supabase.from("engineers").update({ postcodes }).eq("profile_id", currentUser.id);
    setPostcodesSaving(false);
    if (!error) { setPostcodesSaved(true); onProfileFieldsChanged?.({ postcodes }); }
  }
    ...
    <div>
      ...
      {tab === "skills" && (
        <div className="rounded-xl p-5" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          ...
          {canEdit && <p className="text-xs mb-3" style={{ color: C.mid }}>Tick the appliance types you handle. For each active type, click any brands you <strong>won't</strong> repair (highlighted in red) — this matches what your admin sees on your profile.</p>}
          <div className="space-y-2">
            {APPLIANCE_TYPES.map((type) => {
              const active = applianceTypes.includes(type);
              const excludedBrands = brandExclusions[type] || [];
              return (
                <div
                  key={type}
                  className="rounded-lg p-3"
                  style={{ background: active ? C.sidebar : "transparent", border: `1.5px solid ${active ? C.border : "transparent"}` }}
                >
                  <label className="flex items-center gap-2" style={{ cursor: canEdit ? "pointer" : "default" }}>
                    <input type="checkbox" checked={active} disabled={!canEdit} onChange={() => toggleAppliance(type)} />
                    <span className="text-sm font-medium" style={{ color: active ? C.text : C.mid }}>{type}</span>
                    {active && excludedBrands.length > 0 && (
                      <span className="text-xs font-bold" style={{ color: C.danger }}>
                        ({excludedBrands.length} brand{excludedBrands.length > 1 ? "s" : ""} excluded)
                      </span>
                    )}
                  </label>
                  {active && (
                    <div className="mt-2">
                      <div className="text-xs font-bold uppercase mb-1.5" style={{ color: C.light }}>Won't repair — click to toggle:</div>
                      <div className="flex flex-wrap gap-1.5">
                        {(BRANDS[type] || []).filter((b) => b !== "Other").map((b) => {
                          const excluded = excludedBrands.includes(b);
                          return (
                            <button
                              key={b}
                              onClick={() => canEdit && toggleBrandExclusion(type, b)}
                              disabled={!canEdit}
                              className="text-xs rounded px-2 py-1"
                              style={{
                                border: `1.5px solid ${excluded ? C.danger : C.border}`,
                                background: excluded ? C.dangerSoft : "transparent",
          ...
          {canEdit && (
            <div className="mt-4">
              ...
              {skillsSaved && <span className="ml-3 text-sm" style={{ color: C.success }}>Saved.</span>}
            </div>
          )}
        </div>
      )}
      ...
      {tab === "coverage" && (
        <div className="rounded-xl p-5" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          ...
          <div className="flex flex-wrap gap-1 mb-3">
            {postcodes.length ? postcodes.map((p) => (
              <Pill key={p} bg={C.primarySoft} color={C.primary}>
                {p}
                {canEdit && (
                  <button onClick={() => removePostcode(p)} className="ml-1.5" style={{ background: "none", border: "none", cursor: "pointer", color: C.primary, fontSize: 14, padding: 0, lineHeight: 1 }}>×</button>
                )}
              </Pill>
            ...
            <>
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  placeholder="e.g. M1 or LS"
                  value={newPostcode}
                  onChange={(e) => setNewPostcode(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && addPostcode()}
                />
                <button onClick={addPostcode} className="px-4 py-2 rounded-lg text-sm font-bold" style={{ background: C.primary, color: C.sidebar }}>+ Add</button>
              </div>
              <button onClick={savePostcodes} disabled={postcodesSaving} className="px-4 py-2 rounded-lg text-sm font-bold" style={{ background: C.primary, color: C.sidebar, opacity: postcodesSaving ? 0.7 : 1 }}>
                {postcodesSaving ? "Saving…" : "Save Coverage"}
              </button>
              {postcodesSaved && <span className="ml-3 text-sm" style={{ color: C.success }}>Saved.</span>}
            </>
          )}
        </div>
      )}
...
export default function App() {
  ...
  // both call sites.
  useEffect(() => {
    if (currentUser?.engineerRowId) loadBookings(currentUser.engineerRowId);
  ...
  return (
    <div className="flex min-h-screen font-sans" style={{ background: C.bg, color: C.text }}>
