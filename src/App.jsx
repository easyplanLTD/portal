...
function LoginScreen({ onLogin }) {
  ...
  // Goes through the request-password-reset Edge Function rather than
  // calling supabase.auth.resetPasswordForEmail() directly, so a reset link
  // only goes out for an engineer account — never an owner/staff one, even
  // if someone requests it from here. The function does the profiles lookup
  // server-side (RLS blocks that read from the browser before sign-in) and
  // always resolves the same way either way, so this can't be used to probe
  // which emails exist or what role they hold.
  // The link it contains lands the engineer back here with
  // #access_token=...&type=recovery in the hash, which AUTH_HASH/
  // inviteOrRecoveryType() above already know how to catch — App renders
  // SetPasswordScreen for that case exactly the same way it does for an
  // invite link, so no new landing screen is needed here.
  async function sendReset() {
    setError(""); setResetMsg("");
    if (!email) { setError("Enter your email above first."); return; }
    setLoading(true);
    const { error: resetErr } = await supabase.functions.invoke("request-password-reset", {
      body: { email, app: "portal" },
    });
    setLoading(false);
    if (resetErr) { setError("Something went wrong — please try again."); return; }
