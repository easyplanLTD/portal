...
function WorkloadWidget({ tabs }) {
  ...
  // Nothing left to act on -- swap the tabs for a single congratulatory
  // banner. "View" drops back into the normal tabbed layout (still every
  // tab at zero) for anyone who wants to see the categories rather than
  // just take the banner's word for it.
  if (totalItems === 0 && !expanded) {
    return (
      <div style={{ background: C.successSoft, border: `1.5px solid ${C.success}`, borderRadius: 12, padding: "13px 18px", marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <button onClick={() => setExpanded(true)} style={{ background: "none", border: "none", color: C.success, opacity: 0.8, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>View</button>
      </div>
    );
  }
  ...
  // neutral grey tab pills, so it reads as "still all clear, here's the
  // breakdown" rather than looking like there's suddenly work again.
  if (totalItems === 0 && expanded) {
    return (
      <div style={{ background: C.successSoft, border: `1.5px solid ${C.success}`, borderRadius: 12, padding: "13px 17px", marginBottom: 14 }}>
        ...
        <div style={{ marginTop: 14, textAlign: "center", color: C.success, opacity: 0.8, fontSize: 13 }}>Nothing here — all caught up.</div>
      </div>
    );
  }
