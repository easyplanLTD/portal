...
const APPLIANCE_TYPES = ["Washing Machine", "Fridge/Freezer", "Dishwasher", "Oven/Cooker", "Tumble Dryer", "Hob", "Microwave"];
...
function mapBookingRow(row) {
  return {
    ...
    // Kept separately (not folded into scheduledDate) since the detail
    // modal's date/time inputs edit these two independently -- see
    // BookingDetailModal's own scheduledDate/scheduledTime state below.
    scheduledDateOnly: row.scheduled_date || "",
    scheduledTimeOnly: row.scheduled_time || "",
    ...
    paid: !!row.paid,
    // Customer payment collected by the engineer on the job (see
    // 0008_payment_taken.sql) -- separate from `paid` above, which is the
    // office paying the SP. Must be true before the engineer can mark the
    // job Completed.
    paymentTaken: !!row.payment_taken,
    paymentTakenAt: row.payment_taken_at || null,
    partsNeeded: !!row.parts_needed,
    partsOrdered: !!row.parts_ordered,
    partsArrived: !!row.parts_arrived,
...
// Used for the quick status-change actions (Mark In Progress / Mark
// Completed / Beyond Repair) -- those only ever need to touch status and
// (when completing) completedDate, so this keeps that call site simple.
// onUpdateJob (below) is always called with a full job object for
// convenience on the UI side, but this only pulls out those two fields.
// Note this is *not* an RLS boundary -- "bookings: engineer updates own" is
// a row-level policy (engineer_id must be theirs) with no column
// restriction, and the `authenticated` role has UPDATE on every column an
// engineer needs, including scheduled_date/scheduled_time/parts_needed/
// parts_ordered -- see bookingDetailsPatch below, used by the booking
// detail modal's own save (a separate, wider patch for a different action).
function bookingPatchFromUpdate(updated) {
  ...
  // Only sent when the "Payment Taken" button itself was pressed (not on
  // every status change), so other actions never touch these columns.
  if (typeof updated.setPaymentTaken === "boolean") {
    patch.payment_taken = updated.setPaymentTaken;
    patch.payment_taken_at = updated.setPaymentTaken ? new Date().toISOString() : null;
  }
  return patch;
}

// Builds the patch for the booking detail modal's save action -- the fields
// an engineer fills in themselves once a job is theirs: when the visit is
// scheduled, and whether parts are needed/ordered. Separate from
// bookingPatchFromUpdate above since this is a different action (editing
// details vs. changing status) with its own form state, not a variant of it.
function bookingDetailsPatch({ scheduledDate, scheduledTime, partsNeeded, partsOrdered }) {
  return {
    scheduled_date: scheduledDate || null,
    scheduled_time: scheduledDate ? (scheduledTime || null) : null,
    parts_needed: partsNeeded,
    // Clearing "parts needed" clears "parts ordered" too -- it doesn't make
    // sense for a job to have no parts needed but parts marked as ordered.
    parts_ordered: partsNeeded ? partsOrdered : false,
  };
}
...
// "What needs your attention" widget for the Dashboard -- tabs group
// everything that's on the engineer to act on (new bookings to start or
// decline, jobs mid-repair, jobs stuck on parts, pending time-off) so it's
// all one click away instead of spread across Bookings/Schedule/Settings.
// `tabs` is [{key,label,items:[{key,title,sub,right,onClick}]}].
// Styled to match FixFlow's admin-panel WorkloadWidget exactly (same
// component, same purpose, different app) -- plain inline styles here
// rather than Tailwind utility classes so the two stay pixel-identical
// regardless of either app's Tailwind config.
function WorkloadWidget({ tabs }) {
  ...
  // Nothing left to act on -- same plain card as the busy state below (no
  // separate tinted/bordered treatment, no expand-to-tabs interaction --
  // that was more going on than the "all clear" case needs), just a green
  // checkmark row swapped in for the tabs.
  if (totalItems === 0) {
    return (
      <div style={{ background: C.card, borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,.05)", overflow: "hidden", marginBottom: 14 }}>
        ...
        <div style={{ borderTop: `1px solid ${C.border}`, padding: 22, textAlign: "center", color: C.mid, fontSize: 13 }}>Nothing here — all caught up.</div>
      </div>
    );
  }

  // Still got outstanding queues -- box stays the plain neutral card, same
  // grey inactive pills as the rest of the dashboard; only the *active*
  // pill turns amber/orange (instead of lime) to flag "this needs
  // attention" vs. the all-clear state above, which recolours everything
  // green. Individual row content keeps its own colour-coding as before.
  return (
    <div style={{ background: C.card, borderRadius: 12, boxShadow: "0 1px 3px rgba(0,0,0,.05)", overflow: "hidden", marginBottom: 14 }}>
        ...
        {tabs.map((t) => {
          ...
          return (
            <button key={t.key} onClick={() => setActive(t.key)} style={{
              ...
              background: isActive ? C.warn : "rgba(255,255,255,0.07)",
              color: isActive ? "#000000" : C.mid,
            }}>
              {t.label}
              <span style={{ background: isActive ? "rgba(0,0,0,0.15)" : "rgba(255,255,255,0.09)", color: isActive ? "#000000" : C.mid, borderRadius: 10, padding: "1px 7px", fontSize: 11, fontWeight: 800 }}>{t.items.length}</span>
            </button>
          );
        })}
      </div>
      <div style={{ borderTop: `1px solid ${C.border}` }}>
        {activeTab.items.length === 0
          ? <div style={{ padding: 22, textAlign: "center", color: C.mid, fontSize: 13 }}>Nothing here — all caught up.</div>
          : activeTab.items.map((item) => (
            <div key={item.key} onClick={item.onClick} style={{ padding: "10px 17px", borderBottom: `1px solid ${C.border}`, cursor: item.onClick ? "pointer" : "default", display: "flex", alignItems: "center", gap: 12 }}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {item.sub && <div style={{ color: C.mid, fontSize: 11, marginTop: 1 }}>{item.sub}</div>}
              </div>
              {item.right}
            </div>
          ))}
...
// top of the current page, like BookingDetailModal below) -- a thin lime
// border plus a soft matching glow, same brand colour (C.primary) as the
// primary action buttons, so the edge of the box and its fields are easy to
// make out against the dark background. Ini asked for this after reporting
// the admin panel's "Add SP" popup was too dark to see the edges of; that
// fix made it an opt-in `accent` prop on admin-panel's shared Modal
// component (left off by default so other popups there were unaffected).
// Portal only has the one popup (BookingDetailModal) right now, and Ini
// asked for it to apply to any box that opens, so here it's just the
// default look for a modal card/field -- any future popup should reuse
// these two constants rather than the plain card/input styles, to stay
// covered automatically instead of needing an opt-in flag remembered per
// popup.
const modalCardStyle = { background: C.card, border: `1px solid ${C.primary}`, boxShadow: "0 24px 80px rgba(0,0,0,.4), 0 0 18px rgba(212,255,60,0.15)" };
const modalInputStyle = { ...inputStyle, border: `1.5px solid ${C.primary}` };
...
function DashboardView({ currentUser, jobs, setActiveView }) {
  ...
  // stays keyed on scheduledDate. The "Bookings today" stat card is a
  // different question -- "how many bookings have come to me today" -- which
  // is about when the job was assigned to this engineer, not when it's
  // scheduled for (a booking assigned today might be scheduled for next
  // week, and one scheduled for today might've been assigned days ago).
  // assignedAt (see mapBookingRow above) is stamped by a DB trigger whenever
  // engineer_id is set/changed, so it tracks exactly that.
  const todaysJobs = myJobs.filter((j) => j.scheduledDate && j.scheduledDate.startsWith(todayStr()));
  const newBookingsToday = myJobs.filter((j) => j.assignedAt && j.assignedAt.startsWith(todayStr()));
  ...
  // Active (not yet closed-out) jobs assigned to this engineer that have no
  // scheduled_date/scheduled_time set yet -- i.e. the engineer hasn't told the
  // customer/FixFlow when they're coming. Mirrors profileCompletionGaps'
  // job of surfacing "you still need to do something" on the Dashboard, just
  // keyed on the booking rather than the profile.
  const unscheduledJobs = myJobs.filter((j) => !CLOSED_STATUSES.includes(j.status) && !j.scheduledDate);
  ...
  return (
    <div>
      ...
      {unscheduledJobs.length > 0 && (
        <div className="rounded-xl p-4 mb-6 flex items-center justify-between gap-4" style={{ background: C.orangeSoft, border: `1px solid rgba(251,146,60,0.3)` }}>
          <div>
            <p className="font-medium mb-1" style={{ color: C.orange }}>
              {unscheduledJobs.length === 1 ? "1 booking still needs scheduling" : `${unscheduledJobs.length} bookings still need scheduling`}
            </p>
            <p className="text-sm" style={{ color: C.orange, opacity: 0.85 }}>
              {unscheduledJobs.length === 1
                ? `Contact ${unscheduledJobs[0].customerName} to arrange a visit, then add the date and time to the booking.`
                : `Contact these customers to arrange a visit, then add the date and time to each booking: ${unscheduledJobs.map((j) => j.customerName).join(", ")}.`}
            </p>
          </div>
          <button onClick={() => setActiveView("bookings")} className="text-sm font-bold rounded-lg px-4 py-2 whitespace-nowrap" style={{ background: C.orange, color: "#1a0f00" }}>
            Schedule now
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="rounded-xl p-5" style={{ background: C.card, borderLeft: `4px solid ${C.primary}` }}>
          ...
          <p className="text-3xl font-bold" style={{ color: C.text }}>{newBookingsToday.length}</p>
        </div>
      ...
      <WorkloadWidget tabs={[
        {
          key: "new",
          label: "New Bookings",
          items: myJobs.filter((j) => j.status === "assigned").map((j) => ({
            key: j.id,
            title: `${j.customerName} — ${j.applianceType} (${j.brand})`,
            sub: `${fmtDateTime(j.scheduledDate)} · ${j.address}`,
            right: <Pill bg={STATUS_STYLES[j.status].bg} color={STATUS_STYLES[j.status].t}>{j.status.replace("_", " ")}</Pill>,
        ...
        },
        {
          key: "in_progress",
          label: "In Progress",
          items: myJobs.filter((j) => j.status === "in_progress").map((j) => ({
            key: j.id,
            title: `${j.customerName} — ${j.applianceType} (${j.brand})`,
            sub: `${fmtDateTime(j.scheduledDate)} · ${j.address}`,
            right: <Pill bg={STATUS_STYLES[j.status].bg} color={STATUS_STYLES[j.status].t}>{j.status.replace("_", " ")}</Pill>,
        ...
        },
        {
          key: "parts_awaited",
          label: "Parts Awaited",
          items: myJobs.filter((j) => j.status === "parts_awaited").map((j) => ({
            key: j.id,
            title: `${j.customerName} — ${j.applianceType} (${j.brand})`,
            sub: `${fmtDateTime(j.scheduledDate)} · ${j.address}`,
            right: <Pill bg={STATUS_STYLES[j.status].bg} color={STATUS_STYLES[j.status].t}>{j.status.replace("_", " ")}</Pill>,
        ...
        },
        {
          key: "time_off",
          label: "Time Off",
          items: pendingTimeOff.map((t) => ({
            key: t.id,
            title: `${fmtDateShort(t.startDate)} – ${fmtDateShort(t.endDate)}`,
            sub: t.note || "Awaiting approval",
            right: <Pill bg={C.warnSoft} color={C.warn}>Pending</Pill>,
...
// Opens when an engineer clicks a booking card in BookingsView. Shows the
// full set of customer/job details FixFlow already has (nothing an admin
// wouldn't -- just no admin-only actions like reassigning or deleting), plus
// a small form for the handful of fields an engineer is meant to fill in
// themselves once a job is theirs: when the visit is scheduled, and whether
// parts are needed/ordered. Saves via onSaveDetails (App's saveJobDetails),
// a plain `bookings` update scoped by the "engineer updates own" RLS policy
// -- see bookingDetailsPatch above for exactly what gets written.
function BookingDetailModal({ job, onClose, onSaveDetails, currentUser }) {
  const [scheduledDate, setScheduledDate] = useState(job.scheduledDateOnly || "");
  const [scheduledTime, setScheduledTime] = useState(job.scheduledTimeOnly || "");
  const [partsNeeded, setPartsNeeded] = useState(!!job.partsNeeded);
  const [partsOrdered, setPartsOrdered] = useState(!!job.partsOrdered);
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState("");

  // Engineer Notes -- a running, timestamped log an engineer can add to on a
  // job (separate from `job.notes`/detailRows below, which is FixFlow's own
  // single staff-only "Internal Notes" field). Lives in its own
  // `booking_notes` table (one row per note, see
  // supabase/migrations/0005_booking_notes.sql) rather than a column on
  // `bookings`, since it's an append-only list, not a single field either
  // side overwrites -- and it's what admin-panel's JobDetail now also reads
  // to show these same notes to staff.
  ...
  async function addNote() {
    ...
    if (!body) return;
    setNoteErr(""); setAddingNote(true);
    const { data, error } = await supabase
      .from("booking_notes")
      .insert({ booking_id: job.id, engineer_id: currentUser.engineerRowId, author_name: currentUser.name, body })
      .select("id, body, author_name, created_at")
      .single();
    setAddingNote(false);
    if (error) { setNoteErr(error.message || "Failed to add note. Please try again."); return; }
  ...
  async function save() {
    ...
    try {
      await onSaveDetails(job, { scheduledDate, scheduledTime, partsNeeded, partsOrdered });
      onClose();
    } catch (e) {
  ...
  const detailRows = [
    ["Address", job.postcode ? `${job.address}, ${job.postcode}` : job.address],
    ["Phone", job.phone],
    ["Email", job.email],
    ["Appliance age", job.applianceAge],
    ["Priority", job.priority],
    ["Preferred call time", job.preferredCallTime],
    ["Fault reported", job.faultDescription],
    ["Notes", job.notes],
  ].filter(([, value]) => !!value);
    ...
    >
      ...
      >
        <div className="flex items-start justify-between mb-4 gap-3">
          <div>
            <h2 className="text-lg font-bold" style={{ color: C.text }}>{job.customerName} — {job.applianceType} ({job.brand})</h2>
            <div className="mt-1"><Pill bg={STATUS_STYLES[job.status].bg} color={STATUS_STYLES[job.status].t}>{job.status.replace("_", " ")}</Pill></div>
          </div>
          <button onClick={onClose} aria-label="Close" className="text-lg leading-none" style={{ color: C.mid, background: "none", border: "none", cursor: "pointer" }}>✕</button>
        </div>

        <div className="space-y-1.5 text-sm mb-5">
          {detailRows.map(([label, value]) => (
            <p key={label}><span style={{ color: C.mid }}>{label}:</span> <span style={{ color: C.text }}>{value}</span></p>
          ))}
        </div>
        ...
        <div className="pt-4" style={{ borderTop: `1px solid ${C.border}` }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <Field label="Scheduled time">
              <input type="time" className={inputCls} style={{ ...modalInputStyle, opacity: scheduledDate ? 1 : 0.5 }} value={scheduledTime} onChange={(e) => setScheduledTime(e.target.value)} disabled={!scheduledDate} />
            </Field>
          </div>
          <label className="flex items-center gap-2 text-sm mb-2" style={{ color: C.text }}>
            <input type="checkbox" checked={partsNeeded} onChange={(e) => { setPartsNeeded(e.target.checked); if (!e.target.checked) setPartsOrdered(false); }} />
            Parts needed
          </label>
          <label className="flex items-center gap-2 text-sm mb-1" style={{ color: partsNeeded ? C.text : C.light }}>
            <input type="checkbox" checked={partsOrdered} onChange={(e) => setPartsOrdered(e.target.checked)} disabled={!partsNeeded} />
            Parts ordered
          </label>
          {job.partsArrived && <p className="text-xs mt-1" style={{ color: C.success }}>Parts marked as arrived.</p>}
        </div>

        <div className="pt-4 mt-4" style={{ borderTop: `1px solid ${C.border}` }}>
          <p className="text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: C.mid }}>Engineer Notes</p>
          {notesLoading ? (
            <p className="text-sm mb-3" style={{ color: C.light }}>Loading notes…</p>
          ) : notes.length === 0 ? (
            <p className="text-sm mb-3" style={{ color: C.light }}>No notes yet.</p>
          ) : (
            <div className="space-y-3 mb-3 max-h-40 overflow-y-auto pr-1">
              {notes.map((n) => (
                <div key={n.id}>
                  <p className="text-xs mb-0.5" style={{ color: C.light }}>{n.author_name} · {fmtDateTime(n.created_at)}</p>
                  <p className="text-sm whitespace-pre-wrap" style={{ color: C.text }}>{n.body}</p>
                </div>
              ))}
            </div>
          )}
          ...
          />
          {noteErr && <p className="text-sm mt-2" style={{ color: C.danger }}>{noteErr}</p>}
          <div className="flex justify-end mt-2">
            <button
              onClick={addNote}
              disabled={addingNote || !newNote.trim()}
              className="text-sm font-bold rounded-lg px-4 py-2"
              style={{ background: C.primary, color: C.sidebar, opacity: addingNote || !newNote.trim() ? 0.6 : 1 }}
            >
        ...
        {err && <p className="text-sm mt-4" style={{ color: C.danger }}>{err}</p>}

        <div className="flex gap-2 justify-end mt-5">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ background: "rgba(255,255,255,0.06)", color: C.text }}>Close</button>
          <button onClick={save} disabled={saving} className="px-4 py-2 rounded-lg text-sm font-bold" style={{ background: C.primary, color: C.sidebar, opacity: saving ? 0.7 : 1 }}>
            {saving ? "Saving…" : "Save"}
          </button>
        </div>
      </div>
...
function BookingsView({ currentUser, jobs, onUpdateJob, onSaveJobDetails, onDeclineJob, declineWindowMinutes }) {
  const [tab, setTab] = useState("bookings");
  const [subTab, setSubTab] = useState("active");
  const [declining, setDeclining] = useState(null);
  const [declineErr, setDeclineErr] = useState("");
  const [selectedJobId, setSelectedJobId] = useState(null);
  // Popup shown when "Mark Completed" is pressed before "Payment Taken" (or
  // when a status update fails) -- { title, message } or null.
  const [notice, setNotice] = useState(null);
  const myJobs = jobs.filter((j) => j.engineerId === currentUser.engineerRowId);
  // Looked up by id (rather than keeping the job object itself in state) so
  // the modal always reflects the latest row once the realtime sync in App
  // (or this same modal's own save) updates `jobs` -- e.g. status changing
    ...
    <div>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs mt-1" style={{ color: C.light }}>
                    {!j.scheduledDate && !CLOSED_STATUSES.includes(j.status)
                      ? <span style={{ color: C.orange, fontWeight: 600 }}>Needs scheduling</span>
                      : `Scheduled: ${fmtDateTime(j.scheduledDate)}`}
                    {j.partsNeeded && <span style={{ color: C.warn }}> · Parts {j.partsOrdered ? "ordered" : "needed"}</span>}
                  </p>
                  {subTab === "active" && (
                    <div className="flex gap-2 mt-3 items-center flex-wrap" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button
                        onClick={async () => {
                          const err = await onUpdateJob({ ...j, setPaymentTaken: !j.paymentTaken });
                          if (err) setNotice({ title: "Couldn't update payment", message: err });
                        }}
                        title={j.paymentTaken ? "Payment confirmed — click to undo" : "Confirm the customer has paid"}
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold"
                        style={j.paymentTaken
                          ? { background: C.primary, color: "#000" }
                          : { background: "rgba(212,255,60,0.12)", color: C.primary, border: `1px solid ${C.primary}` }}
                      >
                        {j.paymentTaken ? "✓ Payment Taken" : "Payment Taken"}
                      </button>
                      <button
                        onClick={async () => {
                          if (!j.paymentTaken) {
                            setNotice({
                              title: "Payment not taken yet",
                              message: "This job can't be marked as completed until payment has been taken. Collect payment from the customer, press \"Payment Taken\", then mark the job as completed.",
                            });
                            return;
                          }
                          const err = await onUpdateJob({ ...j, status: "completed", completedDate: new Date().toISOString() });
                          if (err) setNotice({ title: "Couldn't mark as completed", message: err });
                        }}
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold"
                        style={{ background: C.successSoft, color: C.success, opacity: j.paymentTaken ? 1 : 0.55 }}
                      >
                        Mark Completed
                      </button>
      ...
      {selectedJob && (
        <BookingDetailModal
          ...
          onSaveDetails={onSaveJobDetails}
      ...
      {notice && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-lg font-bold mb-2" style={{ color: C.text }}>{notice.title}</h2>
            <p className="text-sm mb-5" style={{ color: C.mid }}>{notice.message}</p>
...
function SettingsView({ currentUser, onAddTimeOff, onRemoveTimeOff, onProfileFieldsChanged }) {
  ...
  const [skillsDraft, setSkillsDraft] = useState(currentUser.applianceTypes || []);
  const [skillsSaving, setSkillsSaving] = useState(false);
  const [skillsSaved, setSkillsSaved] = useState(false);
  const [skillsErr, setSkillsErr] = useState("");
  const [postcodesDraft, setPostcodesDraft] = useState(currentUser.postcodes || []);
  ...
  function toggleSkill(type) {
    if (!canEdit) return;
    setSkillsDraft((cur) => (cur.includes(type) ? cur.filter((t) => t !== type) : [...cur, type]));
    setSkillsSaved(false);
  }

  async function saveSkills() {
    setSkillsErr(""); setSkillsSaving(true);
    const { error } = await supabase.from("engineers").update({ appliance_types: skillsDraft }).eq("profile_id", currentUser.id);
    setSkillsSaving(false);
    if (error) { setSkillsErr(error.message); return; }
    setSkillsSaved(true);
    onProfileFieldsChanged?.({ applianceTypes: skillsDraft });
  }

  function addPostcode() {
    if (!canEdit) return;
    const pc = newPostcode.trim().toUpperCase();
    if (pc && !postcodesDraft.includes(pc)) setPostcodesDraft((cur) => [...cur, pc]);
    setNewPostcode("");
  ...
  function removePostcode(pc) {
    if (!canEdit) return;
    setPostcodesDraft((cur) => cur.filter((x) => x !== pc));
    setCoverageSaved(false);
  }

  async function saveCoverage() {
    // If there's still text sitting in the "add a postcode" box, fold it in rather
    // than silently discarding it -- someone who types a code and hits Save (without
    // first clicking + Add or pressing Enter) should not have that save wipe their list.
    const pending = newPostcode.trim().toUpperCase();
    const toSave = pending && !postcodesDraft.includes(pending) ? [...postcodesDraft, pending] : postcodesDraft;
    setCoverageErr(""); setCoverageSaving(true);
    const { error } = await supabase.from("engineers").update({ postcodes: toSave }).eq("profile_id", currentUser.id);
    setCoverageSaving(false);
    if (error) { setCoverageErr(error.message); return; }
    setPostcodesDraft(toSave);
    setNewPostcode("");
    setCoverageSaved(true);
    onProfileFieldsChanged?.({ postcodes: toSave });
  }
        ...
        <div className="rounded-xl p-5" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          ...
          <div className="space-y-3">
            {APPLIANCE_TYPES.map((type) => {
              const active = canEdit ? skillsDraft.includes(type) : currentUser.applianceTypes.includes(type);
              const excludedBrands = currentUser.brandExclusions[type] || [];
              return (
                <label
                  key={type}
                  className="border rounded-lg p-3 flex items-center gap-2"
                  style={{ borderColor: C.border, opacity: active ? 1 : 0.5, cursor: canEdit ? "pointer" : "default" }}
                >
                  {canEdit && (
                    <input type="checkbox" checked={active} onChange={() => toggleSkill(type)} />
                  )}
                  <div>
          ...
          {canEdit && (
            <div className="mt-4 flex items-center gap-3">
              <button onClick={saveSkills} disabled={skillsSaving} className="px-4 py-2 rounded-lg text-sm font-bold" style={{ background: C.primary, color: C.sidebar, opacity: skillsSaving ? 0.7 : 1 }}>
                {skillsSaving ? "Saving…" : "Save Skills"}
              </button>
              {skillsSaved && <span className="text-xs" style={{ color: C.success }}>Saved</span>}
              {skillsErr && <span className="text-xs" style={{ color: C.danger }}>{skillsErr}</span>}
            </div>
          )}
        </div>
      )}
      ...
      {tab === "coverage" && (
        <div className="rounded-xl p-5" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          ...
          <div className="flex flex-wrap gap-1 mb-3">
            {(canEdit ? postcodesDraft : currentUser.postcodes).length ? (canEdit ? postcodesDraft : currentUser.postcodes).map((p) => (
              canEdit ? (
                <span key={p} className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold" style={{ background: C.primarySoft, color: C.primary }}>
                  {p}
                  <button onClick={() => removePostcode(p)} style={{ background: "none", border: "none", cursor: "pointer", color: C.primary, fontSize: 14, padding: 0, lineHeight: 1 }}>×</button>
                </span>
              ) : (
                <Pill key={p} bg={C.primarySoft} color={C.primary}>{p}</Pill>
              )
            )) : <p className="text-sm" style={{ color: C.mid }}>None set yet.</p>}
          </div>
          {canEdit && (
            <>
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  placeholder="e.g. BN or M1"
                  value={newPostcode}
                  onChange={(e) => setNewPostcode(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && addPostcode()}
                />
                <button onClick={addPostcode} className="px-3 py-2 rounded-lg text-sm font-bold" style={{ background: C.primary, color: C.sidebar }}>+ Add</button>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={saveCoverage} disabled={coverageSaving} className="px-4 py-2 rounded-lg text-sm font-bold" style={{ background: C.primary, color: C.sidebar, opacity: coverageSaving ? 0.7 : 1 }}>
                  {coverageSaving ? "Saving…" : "Save Coverage"}
                </button>
                {coverageSaved && <span className="text-xs" style={{ color: C.success }}>Saved</span>}
                {coverageErr && <span className="text-xs" style={{ color: C.danger }}>{coverageErr}</span>}
              </div>
            </>
          )}
        </div>
...
export default function App() {
  ...
  // Live sync of this engineer's `bookings` rows -- until now Portal only
  // ever refetched after an action the engineer themselves took (updateJob/
  // declineJob above calling loadBookings), so a booking newly assigned to
  // them, or a change made from FixFlow (reassignment, a staff edit), never
  // showed up here without a manual page reload. That's what was behind
  // "Bookings today" (and the schedule/board below it) looking stale or
  // wrong while the app was left open.
  //
  // Filtered to `engineer_id=eq.<their row>` so this engineer's browser only
  // ever receives their own rows over the wire, matching the "bookings:
  // engineer sees own" RLS policy loadBookings already relies on.
  useEffect(() => {
    if (!currentUser?.engineerRowId) return;
    const filter = `engineer_id=eq.${currentUser.engineerRowId}`;
    const channel = supabase
      .channel(`bookings-sync-${currentUser.engineerRowId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "bookings", filter }, (payload) => {
        const row = mapBookingRow(payload.new || {});
        setJobs((prev) => (prev.some((j) => j.id === row.id) ? prev : [...prev, row]));
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "bookings", filter }, (payload) => {
        const row = mapBookingRow(payload.new || {});
        setJobs((prev) => prev.map((j) => (j.id === row.id ? row : j)));
      })
      .on("postgres_changes", { event: "DELETE", schema: "public", table: "bookings", filter }, (payload) => {
        // A DELETE payload's `old` row is normally just the primary key
        // unless the table has REPLICA IDENTITY FULL set -- id is all we
        // need here anyway.
  ...
  async function updateJob(updated) {
    ...
    // Returned (not thrown) so the quick-action buttons can show a message
    // -- e.g. the DB's own "payment must be taken first" guard.
    return error ? (error.message || "Failed to update booking.") : null;
  }

  // Saves the booking detail modal's form (scheduled date/time, parts
  // needed/ordered) -- see bookingDetailsPatch above for the exact patch
  // shape. Throws on failure (unlike updateJob above) so BookingDetailModal's
  // save() can catch it and show the error inline instead of silently
  // closing on a failed write.
  async function saveJobDetails(job, fields) {
    const patch = bookingDetailsPatch(fields);
    const { error } = await supabase.from("bookings").update(patch).eq("id", job.id);
    if (error) throw new Error(error.message || "Failed to save booking details.");
  ...
  return (
    <div className="flex min-h-screen font-sans" style={{ background: C.bg }}>
      ...
      <main className="flex-1 p-8 pb-24 md:pb-8">
        ...
        {activeView === "bookings" && <BookingsView currentUser={currentUser} jobs={jobs} onUpdateJob={updateJob} onSaveJobDetails={saveJobDetails} onDeclineJob={declineJob} declineWindowMinutes={declineWindowMinutes} />}
        {activeView === "settings" && (
