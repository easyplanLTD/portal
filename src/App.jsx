...
function WorkloadWidget({ tabs }) {
  ...
  if (totalItems === 0 && !expanded) {
    return (
      <div
        className="rounded-xl mb-8 p-4 flex items-center justify-between gap-3 flex-wrap"
        style={{ background: C.successSoft, border: `1.5px solid ${C.success}` }}
      >
        <div className="flex items-center gap-2.5">
          <div className="w-[22px] h-[22px] rounded-md flex items-center justify-center flex-shrink-0" style={{ background: C.success }}>
          ...
          <span className="font-bold" style={{ color: C.success }}>All tasks complete!</span>
        </div>
        <button onClick={() => setExpanded(true)} className="text-sm font-semibold" style={{ color: C.success, opacity: 0.8 }}>View</button>
      </div>
    );
  }
  ...
  if (totalItems === 0 && expanded) {
    return (
      <div className="rounded-xl mb-8 p-4" style={{ background: C.successSoft, border: `1.5px solid ${C.success}` }}>
          ...
          {tabs.map((t) => {
            ...
            return (
              <button
                ...
                style={{
                  border: isActive ? "none" : `1.5px solid ${C.success}`,
              ...
              >
                ...
                <span
                  ...
                  style={{ background: isActive ? "rgba(0,0,0,0.15)" : "rgba(74,222,128,0.15)", color: isActive ? "#000000" : C.success }}
                >
                  {t.items.length}
                </span>
              </button>
        ...
        <p className="text-sm text-center mt-3" style={{ color: C.success, opacity: 0.8 }}>Nothing here — all caught up.</p>
      </div>
    );
  }
  ...
  // keeps its own colour-coding as before.
  return (
    <div className="rounded-xl mb-8" style={{ background: C.card, border: `1px solid ${C.border}` }}>
        ...
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => setActive(t.key)}
              className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold"
              style={{ background: isActive ? C.warn : C.warnSoft, color: isActive ? "#000000" : C.warn }}
            >
              {t.label}
              <span
                className="rounded-full px-1.5 text-[11px] font-bold"
                style={{ background: isActive ? "rgba(0,0,0,0.15)" : "rgba(251,191,36,0.2)", color: isActive ? "#000000" : C.warn }}
              >
                {t.items.length}
              </span>
            </button>
      ...
      <div className="mt-3">
        {activeTab.items.length === 0 ? (
          <p className="p-4 text-sm" style={{ color: C.mid }}>Nothing here — all caught up.</p>
        ) : (
          activeTab.items.map((item) => (
            <div
              ...
              style={{ borderTop: `1px solid ${C.border}`, cursor: item.onClick ? "pointer" : "default" }}
            >
              <div className="min-w-0">
                <p className="font-medium truncate" style={{ color: C.text }}>{item.title}</p>
                {item.sub && <p className="text-sm" style={{ color: C.mid }}>{item.sub}</p>}
              </div>
              {item.right}
            </div>
          ))
