...
// When someone follows an invite or password-reset email, Supabase redirects
// here with #access_token=...&type=invite (or type=recovery) in the URL and
// supabase-js has already turned that into a real session by the time this
// runs. We just need to notice the `type` and show a "set your password"
...
function LoginScreen({ onLogin }) {
  ...
  async function attemptLogin() {
    ...
    const { data: engRow, error: engErr } = await supabase.from("engineers").select("*, profile:profiles(name,phone,email)").eq("profile_id", data.user.id).single();
    if (engErr || !engRow) {
      await supabase.auth.signOut();
...
function DashboardView({ currentUser, jobs }) {
  ...
  useEffect(() => {
    async function restore() {
      ...
      if (data?.session?.user) {
        ...
        if (profile?.role === "engineer") {
          const { data: engRow } = await supabase.from("engineers").select("*, profile:profiles(name,phone,email)").eq("profile_id", data.session.user.id).single();
  ...
  return (
    <div className="flex min-h-screen font-sans" style={{ background: C.bg }}>
      ...
      <main className="flex-1 p-8">
        {activeView === "dashboard" && <DashboardView currentUser={currentUser} jobs={jobs} />}
