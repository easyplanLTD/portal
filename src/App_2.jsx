...
function SettingsView({ currentUser, onAddTimeOff, onRemoveTimeOff, onProfileFieldsChanged }) {
  ...
  async function saveSkills() {
    ...
    const { error } = await supabase.from("engineers").update({ appliance_types: applianceTypes }).eq("profile_id", currentUser.id);
    setSkillsSaving(false);
    if (!error) { setSkillsSaved(true); onProfileFieldsChanged?.({ applianceTypes }); }
  }
